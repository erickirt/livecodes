...
This is the core SDK on which others ([Preact](preact.mdx), [React](react.mdx), [Solid](solid.mdx), [Svelte](svelte.mdx), [Vue](vue.mdx) and [Web Components](web-components.mdx) SDKs) are built on top. It is a light-weight, zero-dependencies library that allows creating, embedding and communication with LiveCodes playgrounds. It also allows easily creating links to playgrounds.
...
- `container` (required): `HTMLElement` or a string representing a CSS selector. This is the container where the playground is rendered.
  If not found, an error is thrown (except in [headless mode](./headless.mdx), in which this parameter is optional and can be omitted).
...
- The first parameter ([`container`](#createplayground)) is not an element or not found (by CSS selector), except in [headless mode](./headless.mdx).
...
When multiple sources are provided in the [embed options](#embed-options), each is converted to a [configuration object](../configuration/configuration-object.mdx) and the properties are merged.
...
A utility function that allows compressing the stringified config object (e.g. for [sharing in URL hash](../tutorials/creating-shareable-urls.mdx)). It encodes it in base64 with a few tweaks to make it URI safe.
...
Allows loading the playground from a custom URL (e.g. a [self-hosted app](../features/self-hosting.mdx) or a [permanent URL](../features/permanent-url.mdx)).
...
A [configuration object](../configuration/configuration-object.mdx) or a URL to a JSON file representing a configuration object to load.
...
When set to `true`, the playground is loaded in [headless mode](./headless.mdx).

### `import`

Type: [`string`](../api/interfaces/EmbedOptions.md#import)

A resource to [import](../features/import.mdx) (from any of the supported [sources](../features/import.mdx#sources)).
...
An object that represents [URL Query parameters](../configuration/query-params.mdx), that can be used to configure the playground.
...
A [starter template](../features/templates.mdx) to load. Allowed valued can be found [here](../api/interfaces/EmbedOptions.md#template).
...
Runs the [result page](../features/result.mdx) (after any required compilation for code).
...
[Formats the code](../features/code-format.mdx).
...
Gets a [share url](../features/share.mdx) for the current project.
...
- The optional property `zoom` sets the result page [zoom level](../features/result.mdx#result-page-zoom) (the selected panel must be `result`).
...
Runs project [tests](./../features/tests.mdx) (if present) and gets test results.
...
- `"ready"`: Called when a new project is loaded (including when [imported](../features/import.mdx)) and the playground is ready to run.
...
- `"code"`: Called when the playground "content" is changed (see [`getCode`](./js-ts.mdx#getcode) and [`getConfig`](./js-ts.mdx#getconfig)).

  This includes changes in:

  - Code (in editors)
  - Editor languages
  - [CSS processors](../features/css.mdx#css-processors)
  - [External resources](../features/external-resources.mdx)
  - Project info (e.g. allows adding content in page head and attributes to `<html>` element)
  - [Custom settings](../advanced/custom-settings.mdx) (e.g. allows changing [import maps](../features/module-resolution.mdx#custom-module-resolution))
  - Project title
  - [Test](../features/tests.mdx) code
...
- `"tests"`: Called when tests run and test results are available (see [`runTests`](./js-ts.mdx#runtests)).
...
- `"run": Called when the playground is run.
  ```ts
  (
    event: "run",
    fn: () => void
...
- `"setBroadcastToken"`: Sets [broadcast `userToken`](../features/broadcast.mdx#technical-details).
...
- `"showVersion"`: Logs the current LiveCodes app version, SDK version, git commit SHA, [permanent app URL](../features/permanent-url.mdx) and SDK URL in the browser console.
